...
<head>
 ...
 <!--website-favicon-->
 <link href="<?= ASSETS . THEME ?>images/logo.gif" rel="icon">
 <!--plugin-css-->
...
<section class="login-page pad-tb">
  <div class="v-center m-auto">
    <a href="<?= ROOT ?>" class="d-block text-center mb30">
      <img width="100" height="100" src="<?= ASSETS . THEME ?>images/logo.gif" alt="Logo" class="mega-darks-logo">
    </a>
          ...
          <div class="fieldsets row mt20">
            <div class="col-md-6 form-group v-center">
              <button type="submit" class="lnk btn-main bg-btn">
                Login <span class="circle"></span>
              </button>
            </div>
            <div class="col-md-6 form-group v-center text-right">
              <a href="<?= ROOT ?>authuser/signup" class="psforgt">New user? Sign up.</a>
            </div>
          </div>
...
document.addEventListener('DOMContentLoaded', () => {
    submitForm(
        '#login-form',               // 1️ Form selector
        '#response',                 // 2️ Response div selector
        '<?= ROOT ?>authuser/login', // 3️ Endpoint (PHP controller)
        8000                        // 4️ Timeout (optional)
    );
});
</script>
